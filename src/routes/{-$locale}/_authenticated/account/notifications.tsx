  ...
  head: ({ params }) => {
    ...
    return {
      meta: [
        ...
        {
          name: "description",
          content: isAr ? "إشعاراتك وتحديثاتك من الهيئة." : "Your ENR notifications and updates.",
        },
        ...
        {loading ? (
          ...
          <div className="grid place-items-center rounded-2xl border border-dashed border-[color:var(--color-border-default)] bg-[color:var(--color-background-elevated)] p-12 text-center">
            <p className="text-sm text-[color:var(--color-text-secondary)]">
              {bi("You have no notifications.", "لا توجد إشعارات لديك.")}
            </p>
          </div>
        ) : (
          <ul className="space-y-3">
            {rows.map((n) => (
              <li
                key={n.id}
                className={`rounded-2xl border border-[color:var(--color-border-default)] p-5 ${n.read ? "bg-[color:var(--color-background-elevated)]" : "bg-[color:var(--color-brand-primary-tint)]"}`}
              >
                <p className="font-bold text-[color:var(--color-text-brand)]">{n.title}</p>
                {n.body && (
                  <p className="mt-1 text-sm text-[color:var(--color-text-secondary)]">{n.body}</p>
                )}
