...
function ContactPage() {
  ...
  return (
    <StubPage
      ...
      subtitle={bi("We're here to help — 24/7 customer support.", "نحن هنا للمساعدة — خدمة عملاء على مدار الساعة طوال أيام الأسبوع.")}
    >
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-[color:var(--color-border-default)] bg-[color:var(--color-background-elevated)] p-5">
          <h3 className="font-bold text-[color:var(--color-text-brand)]">{bi("Hotline", "الخط الساخن")}</h3>
        ...
        <div className="rounded-2xl border border-[color:var(--color-border-default)] bg-[color:var(--color-background-elevated)] p-5">
          <h3 className="font-bold text-[color:var(--color-text-brand)]">{bi("Email", "البريد الإلكتروني")}</h3>
          <p className="mt-2 text-sm" dir="ltr">support@enr.gov.eg</p>
        </div>
        <div className="rounded-2xl border border-[color:var(--color-border-default)] bg-[color:var(--color-background-elevated)] p-5">
          <h3 className="font-bold text-[color:var(--color-text-brand)]">{bi("Head office", "المقر الرئيسي")}</h3>
