  ...
  return (
    ...
    >
      ...
      >
        <input
          required
          placeholder={bi("Your name", "اسمك")}
          aria-label={bi("Your name", "اسمك")}
          className="h-11 rounded-lg border border-[color:var(--color-border-default)] px-3 text-sm"
        />
        <input
          required
          placeholder={bi("Contact email", "البريد الإلكتروني للتواصل")}
          aria-label={bi("Contact email", "البريد الإلكتروني للتواصل")}
          type="email"
          className="h-11 rounded-lg border border-[color:var(--color-border-default)] px-3 text-sm"
        />
        <input
          required
          placeholder={bi("Route (e.g. Cairo → Luxor)", "المسار (مثال: القاهرة ← الأقصر)")}
          aria-label={bi("Route", "المسار")}
          className="h-11 rounded-lg border border-[color:var(--color-border-default)] px-3 text-sm"
        />
        <input
          required
          type="date"
          aria-label={bi("Date of loss", "تاريخ الفقد")}
          className="h-11 rounded-lg border border-[color:var(--color-border-default)] px-3 text-sm"
        />
        <textarea
          required
          placeholder={bi("Describe the item", "صف الشيء المفقود")}
          aria-label={bi("Describe the item", "صف الشيء المفقود")}
          rows={4}
          className="rounded-lg border border-[color:var(--color-border-default)] p-3 text-sm"
        />
        <button
          type="submit"
          className="h-11 rounded-lg bg-[color:var(--color-brand-primary)] text-sm font-semibold text-white"
        >
